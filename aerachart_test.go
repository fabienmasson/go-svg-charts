...
func TestAreaChart(t *testing.T) {
	...
	file, err := os.Create("examples/areachart.svg")
	if err != nil {
...
func TestAreaChartBezier(t *testing.T) {
	...
	lc := charts.NewAreaChart(
		...
		SetYaxisLegend("Net growth").
		SetShowMarkers(true).
		SetInteractive(true).SetBezier(true)

	file, err := os.Create("examples/areachartbezier.svg")
	if err != nil {
		t.Errorf("os.Create error: %s", err)
	}
	defer file.Close()
